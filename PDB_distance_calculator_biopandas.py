...
df = pd.read_excel("/Users/ananthansadagopan/Documents/ChoudharyLab/PTMs/rcsb_pdb_ligand_20220525122521_high_affinity.xlsx")
...
while ww<len(all_pdbs):
    ...
    for temp_residue in residues_to_iterate:
        ...
        if continue_val == 1:
            q=0
            while q<len(chain_id_temp):
                try:
                    temp_sasa.append(struct[0][chain_id_temp[q]][resi_number_temp[q]][atom_name_temp[q]].sasa)
                except:
                    break
                q=q+1
        sasa_score.append(temp_sasa)
    ...
    sasa_score = list(chain.from_iterable(sasa_score))

    df_out = pd.DataFrame([resi_type_ref, chain_ref, resi_ref, min_dist_list, atom_ref, xvals_ref, yvals_ref, zvals_ref, sasa_score]).T
    df_out.columns = ['Residue_Type', 'Chain', 'Residue_Number', 'Min_Distance_from_Reactive_Atom_to_Ligand', 'Reactive_Atom_Reference', 'Reactive_Atom_X', 'Reactive_Atom_Y', 'Reactive_Atom_Z', 'Reactive_Atom_SASA_Ang_squared']
